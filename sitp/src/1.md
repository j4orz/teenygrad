...
does to build a question answering natural language interface on top of a database containing chemical data from the Apollo-11 mission.

Although we might not know *exactly* how that's done, we might be somewhat familiar with the distinction of
"lower level" vs "higher level" languages where the former is closer to the machine (whatever that may mean) such as C++
...
but extend them one level higher by applying them to the natural language of English itself?<br>

With the approach settled, the problem reduces down into answering the question of **how to implement a compiler?**
Depending on your appetite dear reader, you can try to tackle this question from first principles as well.
...
At a high level, a minimal `compile()`r has a 2 step process:
...
The two conversion steps<span class="sidenote-number"></span><span class="sidenote">*With all industrial language implementations, there is almost always a third step in between the two, which optimizes the program.*</span> have fancy names: *parsing* and *generating* respectively.
Converting such steps into Python functions we have the following:

```python
def compile(source: str) -> str:
  abstract_syntax = parse(input)
  target = generate(abstract_syntax)
  return target
...
The next set of questions the problem reduces down to are then:
1. **what IR will you parse the source language of English to?**
2. **what target language will you generate from the IR?**

> [!WARNING]
> Pause and think!

**What if we parsed English to logic as an IR?**<br>
**And then, what if we generated database queries as the target language from said IR?**<br>
To no surprise, that's effectively what the `LUNAR` system does.
Again, following the Design Recipe, let's start with examples:

```python
def lunar(source: str) -> str:
  """
  >>> lunar("What's the average sample size of such")
  '42'
  """
  abstract_syntax = parse_to_logic(input)
  target = generate_to_dbquery(abstract_syntax)
  return target

def parse_to_logic(concrete_syntax: str) -> Tree:
  raise NotImplementedError("")

def generate_to_dbquery(abstract_syntax: str) -> str:
  raise NotImplementedError("")
```
...
### 0.3.1 From Natural Language to Formal Language of Logic

### 0.3.2 From Logic to Queries
