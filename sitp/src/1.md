...
- [**1. Representing *Data* with High Dimensional Stochasticity in `numpy`**](#1-representing-data-with-high-dimensional-stochasticity-in-torch)
  - [**1.1 From Software 1.0 to Software 2.0: Propositionally Discrete to Probabilistically Dimensional**](#11-from-software-10-to-software-20)
...
- [**3. Accelerating *Functions* and *Data* on `CPU` in `teenygrad`**](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  - [**3.1 From Abstract Linear Algebra to Numerical Linear Algebra: Real Arithmetic to Floating Point Arithmetic**](#31-from-abstract-linear-algebra-to-numerical-linear-algebra-real-arithmetic-to-floating-point-arithmetic)
  ...
  - [**3.4 From Virtual Interpreters to Physical Interpreters: Instruction Sets, Pipelines, and Hierarchies**](#34-from-virtual-interpreters-to-physical-interpreters-instruction-sets-pipelines-and-hierarchies)
  - [**3.5 Accelerating the `BLAS` on `CPU` with `Rust` and `RISC-V` via Rooflines**](#35-accelerating-gemm-on-cpu-with-rust-and-risc-v-via-rooflines)
  - [3.6 Accelerating Computation with Instruction Level Parallelism via Loop Unrolling]()
  - [3.7 Accelerating Computation with Data Level Parallelism via Vector/Matrix Extensions]()
  - [3.8 Accelerating Communication with Memory Hierarchies via Register and Cache Blocking]()
  - [3.9 Accelerating Computation with Thread Level Parallelism]()
...
```python
{{#include ../../teeny/examples/1.6-spaces.py:1:5}}
```
...
```python
{{#include ../../teeny/examples/1.6-spaces.py:5:71}}
```
...
## 2. Learning *Functions* from *Data* with Optimization in `numpy`
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
...
### 3.1 From Abstract Linear Algebra to Numerical Linear Algebra: Real Arithmetic to Floating Point Arithmetic
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>

After SITP's first two chapters, you now understand how to write software 2.0 programs which *learn* with `numpy`, and it's core data structure of the multidimensional `np.ndarray`.
For data, instead of using *locally discrete* representations, software 2.0 programs use *high dimensional* stochastic representations.
— this is why Chapter 1 is titled [**1. Representing *Data* with High Dimensional Stochasticity in `numpy`**](#1-representing-data-with-high-dimensional-stochasticity-in-torch)
For functions, instead of constructing maps with explicit instructions, software 2.0 programs recover such maps with differential optimization
— this is why Chapter 2 is titled [**2. Learning *Functions* from *Data* with Parameter Estimation in `numpy`**](#2-learning-functions-from-data-with-optimization-in-numpy)
...
In this chapter, we will finally understand how `numpy` works under the hood by implementing our very own `np.ndarray` with `teenygrad.Tensor`,
and our very own subset of the BLAS with `teenygrad.eagkers`.
...
you will evolve `teenygrad`'s implementation by adding neural network primitives, gradient-based optimization, and gpu acceleration
in order to support the various inductive biases of deep neural networks which were explored in the "era of research" (roughly spanning from 2012-2020).

Before we begin however, there is another transition we must make moving from *programming machine learning algorithms* to *programming machine learning systems* themselves.
And that transition is a shift in mindset, understanding that the linear algebra evaluated with computation is actually *aspirational*.
The first place to understand that is in transitioning from real number arithmetic to floating point arithmetic.

Although machine learning heavily relies on the language of linear algebra, as you now know,
the primary vector spaces in which the discipline of machine learning uses are d-dimensional coordinate spaces  $\reals^d$.

$$
\reals^2 := \{(x,y): x,y \in \reals \} \\
\reals^3 := \{(x,y, z): x,y,z \in \reals \} \\
\reals^d := \{(x_1,x_2, \dots, x_d): x_1,x_2, \dots, x_d \in \reals \} \\
% \begin{aligned}
% \end{aligned}
$$
...
```python
class Rd():
  def __init__(self: Self, data: list[Real]) -> None:
    self.data = data
...
so in reality..
```python
class Floatd(): # class Rd():
  def __init__(self: Self, data: list[float]) -> None: # def __init__(self: Self, data: list[Real]) -> None:
    self.data = data
...
### 3.4 From Virtual Interpreters to Physical Interpreters: Instruction Sets, Pipelines, and Hierarchies
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>

### 3.5 Accelerating `GEMM` on `CPU` with `Rust` and `RISC-V` via Rooflines
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
...
### 3.6 Accelerating Computation of `GEMM` with Instruction Level Parallelism via Loop Unrolling
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
<!-- loop unroll -->

### 3.7 Accelerating Computation of `GEMM` with Data Level Parallelism via Vector/Matrix Extensions
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
<!-- simd -->

### 3.8 Accelerating Communication of `GEMM` with Memory Hierarchies via Register and Cache Blocking
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
<!-- cache blocking -->

### 3.9 Accelerating Computation of `GEMM` with Thread Level Parallelism
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
<!-- tlp -->