...
def parse(english_source: str) -> Tree:
  raise NotImplementedError("")
...
Before we implement the function bodies for `parse()` and `generate()` so that tests pass,
we should figure out what kind of intermediate representation the abstract syntax will use,
thus determining what data structure the parser parses *to* and the generator generates *from*.
...
Now that we've decomposed `lunar()` into `parse` and `generate`,
let's implement the two functions so that the tests pass.
