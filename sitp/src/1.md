...
- [**3. Accelerating *Functions* and *Data* on `CPU` in `teenygrad` with `rust` and `RISC-V`**](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  ...
  - [**3.4 Accelerating the `BLAS` on `CPU`s with the `SISD` Programming Model: Instruction Sets, Pipelines and Hierarchies**](#34-accelerating-the-blas-instruction-sets-pipelines-and-hierarchies)
...
### 3.4 Accelerating the `BLAS` on `CPU`s with the `SISD` Programming Model: Instruction Sets, Pipelines and Hierarchies
