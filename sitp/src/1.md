...
what the two intellectual traditions share in common is that in their original conception, they both focused on the activity of describing reality with *discrete structures* and *deterministic descriptions*.
However, the programmer interested in learning the skills required to build artificial intelligence
must make the transition that has already been made a few centuries ago by our physicist cousins; the transition towards describing reality with *continuous structures* and *stochastic descriptions*.
...
Let's review the kind of methods (and more importantly their limitations) with discrete structures and deterministic descriptions used in natural language processing systems such as ELIZA
...
rules = [
    ...
    (r"It seems that (.*)",
     ["What makes you think {0}?",
      "Why does it seem that {0}?"]),
    (r"(.*) hate (.*)",
     ["Why do you think {0} hates {1}?",
...
def respond(text: str) -> str:
    for pattern, responses in rules:
        if m := re.fullmatch(pattern, text, re.IGNORECASE):
            return random.choice(responses).format(*m.groups())

print(respond("I am very unhappy these days"))
# How long have you been very unhappy these days?

print(respond("It seems that you hate me"))
# What makes you think you hate me?
```
...
> The ELIZA program itself is merely a *translating processor* in the technical programming sense. Gorn [2] in a paper on language systems says: <br> Given a language which already possesses *semantic* content, then a translating processor, even if it operates only *syntactically*, generates corresponding expressions of another language to which we can attribute as "meanings" (possibly multiple — the translator may not be one to one) the "semantic intents" of the generating source expressions; whether we find the result consistent or useful or both is, of course, another problem.<span class="sidenote-number"></span><span class="sidenote">*Weizenbaum also conveys the same conclusion to the public in a [1978 interview](https://www.youtube.com/watch?v=GxSJQnWzJOs): "Well, I would deny that that there's any important sense, non-negligible sense in which the program understands. It certainly creates the illusion of understanding. there's no question about that. But we have to understand that that illusion is an attribution that the person conversing with the program contributes to the conversation. It's not a function of the program itself."*</span>


quitessential example of a rules-based expert system. GOFAI.
