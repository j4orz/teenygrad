...
for instance, why their mother hates them, our implementation immediately fails:
...
def eliza(input: str) -> str:
  ...
  >>> eliza("Why does my mother hate me?")
  'Why do you say your mother hates you?' # <--- FAIL
  """
  if input === "What's the meaning of life?": return "42"
  elif input === "I am unhappy": return "Why do you say you are unhappy?"
  elif input === "I am upset": return "Why do you say you are upset?"
...
so that `eliza` produce reuse such words in it's answer.
...
As an escape hatch for `eliza()` lack of semantic understanding — that is, no word meaning —
any phrase uttered which the regular expression does not recognize will be responded with
`"Please go on."`<span class="sidenote-number"></span><span class="sidenote">*In the next chapter we will reproduce a natural language processing system with a stronger form of syntactic and semantic analysis.*</span>.
...
def eliza(input: str) -> str:
  ...
  """
  if input === "What's the meaning of life?": return "42"
  elif m := re.fullmatch("I am (.*)", input, re.IGNORECASE):
    return "Why do you say you are {0}?".format(*m.groups())
  else: return "Please go on."

print(eliza("I am very unhappy these days")) # Why do you say you are unhappy?
```

Finally, let's lift our transformation rules into `list[tuple[str, list[str]]]`, including the hardcoded response to the meaning of life

```python
import re, random

transformation_rules = [
  (r"What's the meaning of life?", ["42"])
...
> [!IMPORTANT]
> The system will not work by simply adding another if-then rule.
> ELIZA is trying to **describe a reality with too many parts to count**.
...
ELIZA has effectively no internal representation, and no reasoning, something that minsky/mccarthy/newell/simon were staunch advocates

### 0.3 Wood's Interlingua with the Natural Language Compilation of `LUNAR`

ELIZA no internal representation. no reasoning.
...
1. historical (present the paper)
2. principal carnap, montague, frege
3. 3 tier compiler architecture
4. vibecode
