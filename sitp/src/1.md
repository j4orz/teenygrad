...
- [**3. Accelerating *Functions* and *Data* on `CPU` in `teenygrad`**](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  ...
  - [**3.5 Accelerating `AXPY` and `GEMV` on `CPU` via Rooflines 🔄**](#35-accelerating-axpy-and-gemv-on-cpu-with-rust-and-risc-v-via-rooflines)
  - [3.6 Accelerating Computation of `GEMM` with Instruction Level Parallelism via Loop Unrolling]()
  - [3.7 Accelerating Computation of `GEMM` with Data Level Parallelism via Vector Extensions]()
  - [3.8 Accelerating Communication of `GEMM` with Hierarchies via Register and Cache Blocking]()
...
— this is why Chapter 2 you learned the technique of [**2. Learning *Functions* from *Data* with Parameter Estimation in `numpy`**](#2-learning-functions-from-data-with-optimization-in-numpy)
...
there is still exists a large difference between the two, which primarily boils down to *performance*.
...
underneath the hood it's core basic linear algebra subroutines accelerated using native software library called,
well, the set of basic linear algebra subroutines (abbreviated here on in as the BLAS).
That is, **programs whose instructions are executed by the physical machine, rather than a virtual machine**.
In order to accelerate `tg.Tensor`, we must dive deeper into the soul of the machine, gaining mechanical empathy for what's happening underneath the hoood.
...
### 3.5 Accelerating `AXPY` and `GEMV` on `CPU` with `Rust` and `RISC-V` via Rooflines
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
