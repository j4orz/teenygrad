...
And of course, we will be **vibecoding** our software 2.0 with the software 3.0 provided by industrial-grade LLMs themseves.
...
- [1. Syntactic Sequence Learning via n-gram Models with `teenygrad`]()
  ...
  - [1.2 Vibecoding with Software 3.0](#12-distributing-data-stochastically-with-probabilities)
...
> [!IMPORTANT]
> Going through such an exercise is now equivalent to a young child evaluating arithmetic manually without a calculator to *understand* positional-based representations, or a highschool student evaluating derivatives and integrals manually without Wolfram to *understand* evaluating change and areas with infinitesmals and infinities. Feel free to prompt an LLM to implement such a function and analyze it's artifacts post-hoc. What you choose to outsource to an LLM is up to you. A good heuristic to follow is to not outsource understanding.

First, words will be represented with a crude Python `string`.
...
result1 = prog.fullmatch(r"I am unhappy") # returns re.Match
result2 = prog.fullmatch(r"You are unhappy") # returns None
```
...
```python
result = re.fullmatch(r"I am (.*)", "I am unhappy")
result = re.fullmatch(r"I am (.*)", "I am upset")
```
...
### 1.2 Vibecoding with Software 3.0
