...
As briefly mentioned in the [prologue](./pro.md),
although the discipline of artificial intelligence is similar to those neuroscience and psychology
in that they are all sciences of the mind, how the former differs from the latter is in the *methods* used to conduct their experiments.
As its name suggests, artificial intelligence is a science of the artificial<span class="sidenote-number"></span><span class="sidenote">*Herbert Simon...*</span>
which heavily relies on computation, since it can be directed and *designed* by a pattern of rules called a *program*.
That is, artificial intelligence like **ChatGPT** is "just" another computer program implemented in a language called **PyTorch**.

In this wonderful whimsical whirwild tour, we will explore both the *continuous objects* and *stochastic descriptions* themselves
in which are used to program deep neural networks and deep learning frameworks such as ChatGPT and PyTorch respectively.
Over the course of three booklets we will build from scratch our own chat assistant following **[karpathy/nanogpt]()**
and our own deep learning framework following **[j4orz/teenygrad]()**.
Because of this, from the beginning this book assumes basic competence with the art of computer programming.
Although mastery is not required (and as a matter of fact, is not expected, given that true mastery is a life-long process),
you should be familiar with basic programs using features such as numbers, strings, variables, conditionals, loops, functions, and lists
with perhaps some elementary data structures including tables, trees/graphs, and maps/sets.

If you need or want to strengthen your foundations in programming,
you are encouraged and recommended to study **[A Data-Centric Introduction to Computing](https://dcic-world.org/)**,
namely the first three booklets:
1. [Introduction to Programming](https://dcic-world.org/2025-08-27/booklet_intro-to-programming.html)
2. [From Pyret to Python](https://dcic-world.org/2025-08-27/booklet_pyret-to-python.html)
3. [Programming with State](https://dcic-world.org/2025-08-27/booklet_programming-with-state.html)

in which you will learn how to design programs with the pythonic programming language of
**[Pyret](https://pyret.org/)**<span class="sidenote-number"></span><span class="sidenote">*Alternatively, if you are an experienced programmer that is new to the Python language, the official [Python Tutorial](https://docs.python.org/3/tutorial/index.html) is a great resource next to your favorite large language model.*</span>, and slowly migrate to [**Python**]().


(which ends off with pandas)
history of scientific computing with python.
the big 3 are numpy, pandas, and matplotlib
(todo...matlab...r...python...)
...
<!-- 3. BLAS -->
- [1. Representing *Data* with High Dimensional Stochasticity in `numpy`](#1-representing-data-with-high-dimensional-stochasticity-in-numpy)
  - [**1.1 From Software 1.0 to 2.0: Propositionally Discrete to Probabilistically Dimensional ✅**](#11-from-software-10-to-20-propositionally-discrete-to-probabilistically-dimensional)
  - [From Notional to Virtual Machines??]()
  - [**1.2 Distributing Data Stochastically with Probabilities of Probability Spaces 🔄**](#12-distributing-data-stochastically-with-probabilities-of-probability-spaces)
...
- [2. Learning *Functions* from *Data* with Parameter Estimation in `numpy`](#2-learning-functions-from-data-with-parameter-estimation-in-numpy)
...
- [3. Accelerating *Functions* and *Data* on `CPU` in `teenygrad`](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  - [**3.1 From Abstract to Numerical Linear Algebra: Real to Floating Arithmetic 🔄**](#31-from-abstract-to-numerical-linear-algebra-real-to-floating-arithmetic)
  ...
  - [**3.4 From Virtual to Physical Machines: `Rust` Translation and `RISC-V` Evaluation 🔄**](#34-from-virtual-to-physical-machines-rust-translation-and-risc-v-evaluation)
  - [**3.5 Accelerating `AXPY` and `GEMV` on `CPU` via Rooflines 🔄**](#35-accelerating-axpy-and-gemv-on-cpu-via-rooflines)
...
## 1. Representing *Data* with High Dimensional Stochasticity in `numpy`
...
```python
{{#include ../../teeny/examples/1.6-spaces.py:112:}}
```
...
## 2. Learning *Functions* from *Data* with Parameter Estimation in `numpy`
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
