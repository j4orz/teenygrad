...
- [1. Syntactic Sequence Learning](#1-syntactic-sequence-learning)
  ...
  - [1.2 Language as a Stochastic Process](#12-language-as-a-stochastic-process)
  - [1.3 From Variables to Random Variables](#13-from-variables-to-random-variables)
  - [1.4 The Inference of Parameter Estimation]() <!-- Statistics -->
...
### 1.2 Language as a Stochastic Process
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
The first set is the set of all possible <span class="defnote">**outcomes $\omega$**</span>**outcomes** of the experiment called the <span class="defnote defnote--def">**sample space $\Omega$**</span>**sample space**, denoted by $\omega$ and $\Omega$ respectively,
corresponding to all dots in the diagram above.
For instance, modeling the "experiment" or phenomena of language with probability theory, the formal language theoretic vocabulary is now seen as the sample
...
Finally, the function is the <span class="defnote">**probability law $\mathbb{P}: \mathcal{F} \to [0,1]$**</span>**probability law** with type $\mathbb{P}: \mathcal{F} \to \reals$, mapping any event in the event space to a number on the real number line.
...
a set relative to the sample space.** For instance, this measure can be a counter that counts, a ruler measuring length, or an integral measuring area. -->
...
a <span class="defnote defnote--def">**probability space $(\Omega, \mathcal{F}, \mathbb{P})$**</span>**probability space** $(\Omega, \mathcal{F}, \mathbb{P})$,
...
$$
\begin{aligned}
\mathbb{P}(\{\text{banana}, \text{bongo}\}) &= \mathbb{P}(\{\text{banana}\}) + \mathbb{P}(\{\text{bongo}\})
                                            && \text{[by additivity]}\\
&= 0.1 + 0.2 \\
&= 0.3
\end{aligned}
