...
- [**3. Accelerating *Functions* and *Data* on `CPU` in `teenygrad`**](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  ...
  - [3.2 Virtualizing ND Logical `Tensor.shape`s to 1D Physical `Tensor.storage`s with `Tensor.stride`s]()
  - [3.3 Implementing Basic Linear Algebra Subroutines with `Tensor.__add__`, and `Tensor.__matmul__`]()
  ...
  - [**3.5 Accelerating the `BLAS`' `AXPY`, and `GEMV` on `CPU` with `Rust` and `RISC-V` via Rooflines**](#35-accelerating-gemm-on-cpu-with-rust-and-risc-v-via-rooflines)
...
### 3.4 From Virtual Interpreters to Physical Interpreters with `RISC-V`: Instruction Sets, Pipelines, and Hierarchies
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>

![](./assets/boomcore-block-diagram.webp)

### 3.5 Accelerating the `BLAS`' `AXPY`, and `GEMV` on `CPU` with `Rust` via Rooflines
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
