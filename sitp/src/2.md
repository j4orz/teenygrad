...
- [**5. Accelerating *Sequence Models* on `GPU` in `teenygrad`**](#5-accelerating-sequence-models-on-gpu-in-teenygrad-with-cuda-rust)
  ...
  - [**5.4 From Multi-Core Latency-Oriented Processors to Many-Core Throughput-Oriented Processors**](#54-from-multi-core-latency-oriented-processors-to-many-core-throughput-oriented-processors)
  - [**5.5 Accelerating `GEMM` on `GPU` with `CUDA Rust` and `PTX` via Rooflines**](#55-accelerating-gemm-on-gpu-with-cuda-rust-and-ptx-via-rooflines)
...
### 5.4 From Multi-Core Latency-Oriented Processors to Many-Core Throughput-Oriented Processors
<small>[$\hookleftarrow$ Table of Contents](#ii-neural-networks-1)</small>


### 5.5 Accelerating `GEMM` on `GPU` with `CUDA Rust` and `PTX` via Rooflines
<small>[$\hookleftarrow$ Table of Contents](#ii-neural-networks-1)</small>
...
### 5.8 Accelerating _ of `GEMM` on GPU with Tensor Cores
<small>[$\hookleftarrow$ Table of Contents](#ii-neural-networks-1)</small>
