...
- [1. Representing *Data* with Highly Dimensional Stochastic Distributions in `torch`](#1-representing-data-with-highly-dimensional-stochastic-distributions-in-torch)
  ...
  - [1.6 The (Linear) Algebra of Coordinates]()
  - [1.7 Probability + Linear Algebra = High Dimensional Stochastic Distributions](#17-probability--linear-algebra--high-dimensional-stochastic-distributions)
...
In the last three chapters you've started the transition from deductive software 1.0 to inducive software 2.0
by distributing truth stochastically with the language of probability theory.
In [Chapter 1.2]() you (todo).
In [Chapter 1.3]() you (todo).
In [Chapter 1.4]() you (todo).

Ultimately, we want to represent types as random vectors endowed with high dimensional distributions.
In order to do that, throughout the next two chapters we need to introduce the wonderful language of linear algebra
— a generalization of linear transformations on the Cartesian coordinate system.
Once we become comfortable with the basics of the high dimensional coordinates and linear transformations,
we can combine the two languages of probability theory and linear algebra together in the final chapter of
in [Chapter 1.7 Probability + Linear Algebra = *High Dimensional Stochastic Distributions*]()
...
For linear algebra, the Peano axiomatization models vector spaces $\mathcal{V}$ with linear transformations which
...
Since all linear tranformations can be encoded as matrices, linear algebra is essentially the study of vectors $v$ and matrices $A$.
