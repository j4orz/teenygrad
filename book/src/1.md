...
- [1. Representing *Data* with Highly Dimensional Stochastic Distributions in `torch`](#1-representing-data-with-highly-dimensional-stochastic-distributions-in-torch)
  - [1.1 From Deductive Software 1.0 of ELIZA to Inductive Software 2.0 of ChatGPT]()
  ...
  - [1.7 The Fundamental Decomposition: Singular Value Decomposition]()
  - [1.8 Random Vectors and their Distributions]() <!-- this is the goal for part 2 -->
- [2. Learning *Functions* from *Data* with Parameter Estimation in `torch`](#2-learning-functions-from-data-with-optimization-in-torch)
...
Ultimately though, in software 2.0 not only do we
*distribute the truth of datatypes stochastically* with probabilities (and subsequently random variables and distributions)
but also *represent the datatypes in higher dimensions*, which means using *random vectors* endowed with *high dimensional joint distributions*.
For the purposes of building large language models, words are represented as random vectors $\mathbf{x}$ in a multidimensional space $\reals^n$.  <!-- according to the distributions of their neighbors. -->
...
```python
{{#include ../../examples/tensor.py:1:7}}
```
...
```python
{{#include ../../examples/tensor.py:8:14}}
```



### 1.6 The Geometry of Coordinates via Inner Products: Norms, Distances, Projections
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>
