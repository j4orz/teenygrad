...
- [3. Programming CPU Accelerated *Basic Linear Algebra Subroutines* with `teenygrad.Tensor`](#3-programming-cpu-accelerated-basic-linear-algebra-subroutines-with-teenygradtensor)
...
In chapter 1, you've initiated the transition from software 1.0 to software 2.0 by:
...
But deep learning is a specific kind of machine learning. And in turn, machine learning, is a specific kind of statistical learning.
In order to understand the deep neural nets trained to model language in [part II](),
we will build the necessary foundation in statistical modeling practical workhorses.

Recall that the primary goal of machine learning is the mechanization of *induction*,
the process of (todo), developed throughout the millenia even before computation by philosophers such as Aristotle, Occam, and Hume.
More formally, given observations<span class="sidenote-number"></span><span class="sidenote">*We stick to the domain neutral terminology of inputs and outputs. However the input-output pairs $(x^{(i)},y^{(i)})$ are also referred to as predictors/response*</span>
comprising some dataset $d= \{(x^{(i)},y^{(i)})\}_{i=0}^n$,
learn some mapping from input space to output space $f\colon \mathscr{X} \to \mathscr{Y}$,
and evaluate such learned mapping $\hat{y} = f(x^{n+1})$ in order to predict new outputs given unseen input.
...
However, roughly speaking, there are two settings of supervised learning depending on the type of the output space $\mathscr{Y}$ that the learned function $f$ must map to
...
For [part I](./1.md), this chapter covers both cases with *linear functions* which are referred to as *linear regression* and *logistic regression*
respectively<span class="sidenote-number"></span><span class="sidenote">*In [part II](./2.md), we will increase the expressivity of our models with *non-linear functions*, which actually builds off of logistic regression, which is also known as the perceptron.*</span>.
...
Let's take a look at some code to see what we exactly mean, starting with the classic hello world example of house price prediction with linear regression
which straddles tools from all three languages of probability theory, linear algebra, and calculus.
...
which means the function to recover has type $f\colon\reals \to \reals$.
...
## 3. Programming CPU Accelerated Basic Linear Algebra Subroutines with `teenygrad.Tensor`
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
