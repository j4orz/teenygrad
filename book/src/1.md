...
todo: training vs inference? parameterizing the function is like a lambda and closure.
Although $f$ is specified generically with respect to any input $\mathbf{x} \in \reals^{d+1}$,
really, the function $f$ is evaluated with a specific input $\mathbf{x}^{(i)}$ in order to produce a prediction
$\hat{y} = f_{\mathbf{w}}(\mathbf{x}^{(i)})$.
- batch with matrix vector. but motivate why (training?).
- notation f(;)vs f_ lambda and closure

Specifying the equation of the linear regression model
...
but the discipline of machine learning prefers representing it's models computationally.
In Python, with the PyTorch package, we have the following:
...
When $f$ is initialized with some random $\mathbf{w}$,
evaluating the function for some prediction $\hat{y}$ will output unintelligible nonsense.
For the function $f$ to evaluate intelligible sensical output,
parameter estimation of $\mathbf{w}$ from the dataset dataset $\mathcal{D}= \{(x^{(i)},y^{(i)})\}_{i=1}^n$ must be conducted.
That is, just like how all the "intelligence" in the
process of a learning a distribution from data comes from the distribution's parameters,
so does too 
...
**Fitting a Line in Higher Dimensions**
...
We can still define the *batched* evaluation over the entire dataset $\mathcal{D}$ with
