...
- [**5. Accelerating *Sequence Models* on `GPU` in `teenygrad` with `CUDA Rust` and `PTX`**](#5-accelerating-sequence-models-on-gpu-in-teenygrad-with-cuda-rust)
  ...
  - [**5.3 Accelerating `GEMM` on Many Core GPUs with the SPMD Programming Model**](#53-accelerating-gemm-on-many-core-gpus)
...
### 5.3 Accelerating `GEMM` on Many Core GPUs with the SPMD Programming Model
